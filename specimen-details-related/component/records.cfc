...
WHERE  where rownum <= 50 and CONTAINS(ft.cat_num, <cfqueryparam cfsqltype="CF_SQL_TEXT" value="#searchText#">, 1) > 0 and ft.collection_object_id = ff.collection_object_id
</cfquery>
<cfelse>
<cfquery name="qryLoc" datasource="user_login" username="#username#" password="#password#">
select #listColumns# from #session.flatTableName# ff where rownum <= 20 and spec_locality like '%Massachusetts%'
</cfquery>
</cfif>
<cfoutput>
	<cfset i = 1>
